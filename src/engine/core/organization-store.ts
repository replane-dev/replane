  ...
  async getAll(params: {currentUserEmail: NormalizedEmail}): Promise<OrganizationInfo[]> {
    const organizationsQuery = this.db
      ...
      .select([
        'organizations.id',
        'organizations.name',
        'organizations.auto_add_new_users',
    ...
    return rows.map(o => ({
      id: o.id,
      name: o.name,
      autoAddNewUsers: o.auto_add_new_users,
  ...
  async create(organization: Organization): Promise<void> {
    await this.db
      ...
      .values({
        ...
        auto_add_new_users: organization.autoAddNewUsers,
...
function mapOrganization(organization: Selectable<Organizations>): Organization {
  return {
    id: organization.id,
    name: organization.name,
    autoAddNewUsers: organization.auto_add_new_users,
