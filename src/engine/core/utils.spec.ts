...
describe('groupBy', () => {
  it('should group items by a simple property', () => {
    interface Person {
      name: string;
      role: string;
    }
    ...
    const adminGroup = result.find(([key]) => key === 'admin');
    const userGroup = result.find(([key]) => key === 'user');

    expect(adminGroup).toBeDefined();
    expect(userGroup).toBeDefined();
    expect(adminGroup![1]).toHaveLength(2);
    expect(userGroup![1]).toHaveLength(2);
    expect(adminGroup![1]).toContainEqual({name: 'Alice', role: 'admin'});
    expect(adminGroup![1]).toContainEqual({name: 'Charlie', role: 'admin'});
    expect(userGroup![1]).toContainEqual({name: 'Bob', role: 'user'});
    expect(userGroup![1]).toContainEqual({name: 'David', role: 'user'});
  });
  ...
  it('should handle all items having the same key', () => {
    ...
    expect(result[0][0]).toBe('same');
    expect(result[0][1]).toEqual([1, 2, 3, 4, 5]);
  });

  it('should handle all items having unique keys', () => {
    const items = [1, 2, 3, 4];
  ...
  it('should group by number keys', () => {
    ...
    const mod0 = result.find(([key]) => key === 0);
    const mod1 = result.find(([key]) => key === 1);
    const mod2 = result.find(([key]) => key === 2);

    expect(mod0![1]).toEqual([3, 6, 9]);
    expect(mod1![1]).toEqual([1, 4, 7, 10]);
    expect(mod2![1]).toEqual([2, 5, 8]);
  });

  it('should group by object keys using stable JSON', () => {
    interface Item {
      id: number;
      category: {type: string; subtype: string};
    }

    const items: Item[] = [
      {id: 1, category: {type: 'food', subtype: 'fruit'}},
      {id: 2, category: {type: 'food', subtype: 'vegetable'}},
      {id: 3, category: {type: 'food', subtype: 'fruit'}},
      {id: 4, category: {type: 'electronics', subtype: 'phone'}},
    ];
    ...
    const fruitGroup = result.find(([key]) => key.type === 'food' && key.subtype === 'fruit');
    const vegetableGroup = result.find(
      ([key]) => key.type === 'food' && key.subtype === 'vegetable',
    );
    const phoneGroup = result.find(
      ([key]) => key.type === 'electronics' && key.subtype === 'phone',
    );

    expect(fruitGroup).toBeDefined();
    expect(vegetableGroup).toBeDefined();
    expect(phoneGroup).toBeDefined();

    expect(fruitGroup![1]).toHaveLength(2);
    expect(fruitGroup![1]).toContainEqual({id: 1, category: {type: 'food', subtype: 'fruit'}});
    expect(fruitGroup![1]).toContainEqual({id: 3, category: {type: 'food', subtype: 'fruit'}});
    expect(vegetableGroup![1]).toHaveLength(1);
    expect(phoneGroup![1]).toHaveLength(1);
  });

  it('should normalize object keys with different property order', () => {
    interface Item {
      id: number;
      value: number;
    }
    ...
    // Both should produce the same key due to stable JSON
    const result = groupBy(items, item => ({a: item.value, b: 'test'}));
  ...
  it('should handle complex nested object keys', () => {
    ...
    const items: Item[] = [
      ...
      {id: 2, metadata: {tags: ['a', 'b'], count: 5}},
      {id: 3, metadata: {tags: ['c', 'd'], count: 3}},
    ];

    const result = groupBy(items, item => item.metadata);

    expect(result).toHaveLength(2);

    const group1 = result.find(([key]) => key.count === 5);
    const group2 = result.find(([key]) => key.count === 3);

    expect(group1![1]).toHaveLength(2);
    expect(group2![1]).toHaveLength(1);
  });

  it('should preserve original item order within groups', () => {
    const items = [
      {id: 1, group: 'A'},
      {id: 2, group: 'B'},
      {id: 3, group: 'A'},
    ...
    const groupA = result.find(([key]) => key === 'A');
    const groupB = result.find(([key]) => key === 'B');

    expect(groupA![1]).toEqual([
      {id: 1, group: 'A'},
      {id: 3, group: 'A'},
      {id: 5, group: 'A'},
    ]);

    expect(groupB![1]).toEqual([
      {id: 2, group: 'B'},
      {id: 4, group: 'B'},
    ]);
  });
  ...
  it('should handle boolean keys', () => {
    ...
    const result = groupBy(items, item => item % 2 === 0);

    expect(result).toHaveLength(2);

    const evenGroup = result.find(([key]) => key === true);
    const oddGroup = result.find(([key]) => key === false);

    expect(evenGroup![1]).toEqual([2, 4, 6]);
    expect(oddGroup![1]).toEqual([1, 3, 5]);
  });
  ...
  it('should handle null and undefined keys', () => {
    ...
    const categoryA = result.find(([key]) => key === 'A');
    const nullCategory = result.find(([key]) => key === null);

    expect(categoryA![1]).toHaveLength(2);
    expect(nullCategory![1]).toHaveLength(2);
  });
  ...
  it('should handle single item', () => {
    ...
    expect(result[0][1]).toEqual([{id: 1, value: 'test'}]);
  });
});
